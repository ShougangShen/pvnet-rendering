...
def randomly_read_background():
    background_lmdb = lmdb.open(os.path.join(cfg.YCB, 'background'), max_readers=1,
                                readonly=True, lock=False, readahead=False, meminit=False)
    with background_lmdb.begin() as txn:
        background_len = txn.stat()['entries']
        background_idx=np.random.randint(0,background_len)
        with background_lmdb.begin(write=False) as txn:
            background=cv2.imdecode(np.fromstring(
                txn.get('{:08d}'.format(background_idx).encode('ascii')),np.uint8),cv2.IMREAD_COLOR)
